...
export function ActiveRunScreen({ onEnd, onCancel, flashRun, runGoal }: Props) {
  ...
  const [showShortRunConfirm, setShowShortRunConfirm] = useState(false);
  ...
  const racePosition = flashRun
    ? getPlayerPosition(flashRun, durationSeconds)
    : 0;
  ...
  const paceSecPerKm =
    distanceMeters > 10 ? (durationSeconds / distanceMeters) * 1000 : 0;
  const paceMm = Math.floor(paceSecPerKm / 60);
  const paceSs = Math.round(paceSecPerKm % 60)
    .toString()
    .padStart(2, "0");
  const paceDisplay = distanceMeters > 10 ? `${paceMm}:${paceSs}` : "—";
  const ghostDelta = runGoal
    ? distanceMeters -
      ((runGoal.distanceKm * 1000) / (runGoal.timeMinutes * 60)) *
        durationSeconds
    : null;
      ...
      {showShortRunConfirm && (
        ...
        >
          ...
          >
            ...
            >
              No KAD will be earned for runs under 100 metres. End anyway?
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <button
                onClick={() => setShowShortRunConfirm(false)}
                style={{
                  height: 48,
                  borderRadius: 50,
                  border: "none",
        ...
        {isWarmingUp && !geoError && (
          <div
            style={{
              ...
              alignItems: "center",
              gap: 10,
              animation: "kadPulse 2s ease-in-out infinite",
            }}
          >
            <span
              style={{
                ...
                borderRadius: "50%",
                background: "#E0F479",
                animation: "kadPulse 1.5s ease-in-out infinite",
              }}
            />
