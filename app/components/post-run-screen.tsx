...
function formatPace(distM: number, sec: number) {
  ...
  return `${Math.floor(secPerKm / 60)}:${Math.round(secPerKm % 60)
    .toString()
    .padStart(2, "0")}`;
}
...
export function PostRunScreen({
  ...
  const hasMultipliers =
    multiplier > 1 || boostMult > 1 || underdogMult > 1 || socialMult > 1;
  ...
  const heroTitle = raceResult
    ? raceResult.dnf
      ? "Distance not\nreached."
      : `${positionSuffix(raceResult.position)} at submission`
    : distKm >= 10
      ? "Incredible\neffort."
      : distKm >= 5
        ? "Solid\nrun."
      ...
      <div style={{ position: "relative", height: 440, overflow: "hidden" }}>
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(ellipse at 50% 55%, rgba(224,244,121,0.3) 0%, transparent 40%)",
          ...
          >
            {[1, 2, 3, 4, 5].map((i) => (
              <KIcon
                ...
                name="sparkle"
                size={14}
                color={i <= stars ? "#E0F479" : "rgba(255,255,255,0.2)"}
                fill={i <= stars ? "#E0F479" : "none"}
                stroke={i <= stars ? 0 : 1.5}
              />
            ))}
          </div>
          <div
      ...
      >
        ...
        {raceResult &&
          (raceResult.dnf ? (
            ...
            >
              ...
              >
                You covered {(raceResult.distanceM / 1000).toFixed(2)} km. Your
                KAD reward still applies.
              </div>
            </div>
          ) : (
            <RacePodium
        ...
        {hasMultipliers && (
          ...
          >
            ...
            {underdogMult > 1 && raceResult && (
              ...
              >
                You finished {positionSuffix(raceResult.position)} — every
                finish counts. 🔥
              </div>
            )}
          </div>
        ...
        >
          ...
          >
            ...
            >
              +{xpEarned}{" "}
              <span style={{ fontSize: 10, color: "rgba(224,244,121,0.6)" }}>
                XP
              </span>
            </span>
          </div>
          ...
          >
            ...
            <span style={{ color: "rgba(224,244,121,0.7)" }}>
              {100 - xpAfter > 0 ? `${100 - xpAfter} XP to ` : ""}
        ...
        {newBadges.map((badge) => (
          ...
          >
            ...
            >
              <KIcon
                name={badge.icon as Parameters<typeof KIcon>[0]["name"]}
                size={22}
                color="#0D0D0D"
              />
            </div>
